...
const updateFaculty = asyncHandler(async (req, res) => {
  ...
  try {
    const facultyToUpdate = await FacultyModel.findById(id);
    ...
    if (name && name !== facultyToUpdate.name) {
      // Check if new name already exists for this university
      const existingFaculty = await FacultyModel.findOne({
        name,
      ...
      while (await FacultyModel.findOne({ slug, _id: { $ne: id } })) {
        slug = `${suffix}-${originalSlug}`;
        suffix++;
      }
