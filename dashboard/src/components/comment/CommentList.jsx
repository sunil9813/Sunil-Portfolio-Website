...
export const CommentList = ({ resourceId, resourceType = "Chapter", showRootEditor = true }) => {
  ...
  const handleCommentSubmit = async () => {
    if (!resourceId || stripHtml(rootComment).trim().length === 0) {
      return;
    }
  ...
  const handleReplySubmit = async (commentId, replyContent) => {
    if (!resourceId || !commentId || stripHtml(replyContent).trim().length === 0) {
      return;
    }
  ...
  const handleUpdateSubmit = async (commentId) => {
    if (!resourceId || !commentId || stripHtml(editValue).trim().length === 0) {
      return;
    }
  ...
  const handleLikeSubmit = async (event, comment) => {
    ...
    if (!resourceId || !comment?.id || !currentUserId || likingIds[comment.id]) {
      return;
    }
...
export const CommentItem = ({
  ...
  const canManageComment = String(comment?.ownerId || "") === String(currentUserId || "");
