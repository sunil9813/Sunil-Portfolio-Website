...
export const Layout = ({ children, title = "" }) => {
  ...
  return (
    <main className={`app-wrapper ${theme === "dark" ? "dark" : "light"}`}>
      <section className="flex gap-5 bg-light-surface1 dark:bg-dark-surface1">
        <div className="w-[15%]">
          <Sidebar />
        </div>
        <div className="w-[85%] px-6 bg-light-surface1 dark:bg-dark-surface1">
          <Header title={title} />
          <div className="w-full text-black dark:text-textcolor">{children}</div>
        </div>
      </section>
    </main>
  );
