...
const updateComment = async ({ commentId, content }) => {
  const response = await axios.put(`${API_URL}${commentId}`, { content }, { withCredentials: true });
...
const toggleCommentLike = async (commentId) => {
  const response = await axios.patch(`${API_URL}${commentId}/like`, {}, { withCredentials: true });
