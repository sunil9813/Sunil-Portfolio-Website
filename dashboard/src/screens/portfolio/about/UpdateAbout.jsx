...
export const UpdateAbout = () => {
  ...
  // Fetch existing intro data
  useEffect(() => {
    dispatch(getIntro(id))
      .unwrap()
      .catch((err) => {
        toast.error(err.message || "Failed to fetch introduction.");
      });
  }, [dispatch]);
  ...
  useEffect(() => {
    if (intro) {
      setPortfolioIntro({
        ...
        emails: Array.isArray(intro.emails) ? intro.emails.map((e) => ({ email: e.email || "" })) : [],
        phones: Array.isArray(intro.phones) ? intro.phones.map((p) => ({ phone: p.phone || "" })) : [],
        languages: Array.isArray(intro.languages) ? intro.languages.map((l) => ({ language: l.language || "" })) : [],
        socialslinks: Array.isArray(intro.socialslinks) ? intro.socialslinks.map((s) => ({ link: s.link || "" })) : [],
      });
  ...
  const handleUpdate = async () => {
    try {
      ...
      // Append all fields, including empty ones, to allow clearing
      ...
      await dispatch(updateIntro({ formData, id })).unwrap();
    ...
    <>
      <StickyHeader>
        ...
        <div className="flexC gap-2">
          ...
          <TertiaryButton onClick={handleUpdate} disabled={isLoading || fetchLoading}>
      ...
      <section className="flex justify-between gap-3 pb-8">
          ...
          <div className="flex justify-between gap-3">
            <Wrapper className="p-5 w-1/2">
              ...
              <div className="input py-3">
                ...
                <div className="relative">
                  <Input type="text" name="position" className="pl-12" value={position} handleChange={handleInputChange} placeholder="Software Engineer" />
        ...
        <div className="w-1/3">
          ...
          <Wrapper className="p-5 my-3">
            ...
            >
              {cv || intro?.cv?.fileName ? (
                <div className="relative w-full flex items-center justify-center">
                  <p className="text-sm">{cv ? cv.name : intro?.cv?.fileName}</p>
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      setCv(null);
