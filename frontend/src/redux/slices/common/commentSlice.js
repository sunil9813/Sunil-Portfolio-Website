...
const getErrorMessage = (error) => {
  return (error.response && error.response.data && (error.response.data.error || error.response.data.message)) || error.message || "An error occurred";
};
...
export const addComment = createAsyncThunk("comments/add", async ({ resourceId, resourceType, content, parentCommentId, rating }, thunkAPI) => {
  try {
    return await commentService.addComment({ resourceId, resourceType, content, parentCommentId, rating });
  } catch (error) {
    return thunkAPI.rejectWithValue(getErrorMessage(error));
  }
});
